  ...
  function consume(line) {
    ...
    if (type.startsWith("item.") && event.item && typeof event.item.type === "string") {
      if (event.item.type === "agent_message" && type === "item.completed" && typeof event.item.text === "string") {
        ...
        const allowedMcp = event.item.type === "mcp_tool_call" && event.item.server === "ahive_repository" && ["list_files", "search_text", "read_text", "git_summary"].includes(event.item.tool);
...
function repositoryMcpArguments(config) {
  ...
  return [
    ...
    "-c", "mcp_servers.ahive_repository.tool_timeout_sec=5",
    "-c", 'mcp_servers.ahive_repository.default_tools_approval_mode="auto"',
    "-c", 'mcp_servers.ahive_repository.enabled_tools=["list_files","search_text","read_text","git_summary"]'
  ];
}
...
export async function readToolActivity(path, runId) {
  ...
  return content.split(/\r?\n/).filter(Boolean).slice(0, 128).flatMap(line => {
    try {
      ...
      if (event.runId !== runId || !["mcp_server", "list_files", "search_text", "read_text", "git_summary"].includes(event.tool)) return [];
